...
class Prediction(nn.Module):
    def __init__(self, args):
        ...
        self.A_learned = torch.Tensor([
            ...
            # [3.5761e-01, 6.3922e-01, 6.2686e-01, 6.3461e-01, 0]
        ])
    ...
    def predict(self):
        ...
        for id in list(old_data.keys()):
            ...
            # true_t = head_time_all[id, :]
            # if true_t.item() > self.time_horizon:
            #     true_t = torch.tensor([0])
            #     true_time_list.append(true_t.item())
            # else:
            #     true_time_list.append(true_t.item())
            #
            # absolute_error = abs(pred_t.item() - true_t.item())
            # absolute_error_list.append(absolute_error)
            # print('absolute error = ', absolute_error)
            # end_time = time.time()
            # running_time = end_time - start_time
            # print('----- predict the event time for the {}-th sample using t(s) = {} -----'.format(id, running_time))
