...
class PP_model_learning(nn.Module):
    ...
    def compute_integration(self, delay_effect, mask_matrix, seq, A_matrix, base_train):
        ...
        for i in range(B):
            ...
            for val in seq[i, :, -1]:
                if 999 > val > time_horizon:
                    time_horizon = val
            # time_horizon = torch.max(seq[i, :, -1])
            ki = time_horizon - seq_delay[i]
            k[i] = ki
        # k = self.time_horizon - seq_delay

        x = torch.sigmoid(1000 * k)
        x = x * mask_matrix_1

        # A_matrix = self.A_matrix.unsqueeze(0).unsqueeze(-1).repeat(B, 1, 1, L).to(self.device)
        ...
        # intensity_int = intensity_int * self.time_horizon
    ...
    def log_likelihood(self, data_time):
        ...
        # delay_effect = delta_matrix_zeros.unsqueeze(0).unsqueeze(-1).repeat(B, 1, 1, L)
        delay_effect = delta_matrix_zeros.unsqueeze(-1).repeat(1, 1, 1, L)

        # delay_effect = self.ground_truth_delta_matrix.unsqueeze(0).unsqueeze(-1).repeat(B, 1, 1, L)  # truth delay
        ...
        # intensity = torch.sum(torch.log(intensity + 1e-12) * mask_matrix, dim=-1).squeeze()
        ...
        # log_likelihood = intensity - intensity_int
        log_likelihood = event_ll - intensity_int
        log_likelihood = torch.sum(log_likelihood, dim=-1).squeeze()

        # return -log_likelihood, delta_matrix_train, event_ll
        # return -log_likelihood, delta_matrix_zeros[-1, :], event_ll  # no delay
    ...
    def optim_log_likelihood(self, data):
        ...
        #                        self.lr, betas=(0.9, 0.999), eps=1e-05)
        masker_params = [
        ...
        # optimizer_masker = torch.optim.SGD(model.masker.parameters(), self.lr_delta)
        # optimizer_a_learner = torch.optim.SGD(model.a_learner.parameters(), self.lr_A)
        # optimizer_mu_learner = torch.optim.SGD(model.mu_learner.parameters(), self.lr_mu)
        ...
        # optimizer_masker = torch.optim.SGD(model.masker.parameters(), lr=self.lr_delta)
        # optimizer_a_learner = torch.optim.SGD(model.a_learner.parameters(), lr=self.lr_A)
        ...
        for iter in range(self.num_iters):
            ...
            # print("last sample learned delay", delta_matrix_train_all[-1])
            # print("last sample learned A", A_matrix_train_all[-1])
            ...
            #
            # params = norm.fit(mu_vector_train_all)
            # print('----- mu_vector_learned -----')
            # print("mean:", params[0])
            # print("stds:", params[1])
    ...
    def optim(self, data, old_data):
        ...
        # plt.savefig('loss_{}T_{}dims_{}seqs_{}iters_hyper.pdf'.format(self.time_horizon,
        #                                                         self.num_dims,
        #                                                         self.num_seqs,
        #                                                         self.num_iters))
